...
#SELECT * FROM producr WHERE price > 50000;
...
#
# SELECT * FROM product WHERE price < 50000;
# Product.objects.filter(price__ls=50000)
...
# Product.objects.filter(category_id_in=['smartphones', 'accessories'])
#
# SELECT * FROM product WHERE category_id = 'smartphones' OR category_id = 'accessories';
# Product.objects.filter(Q(category_id='smartphones')|Q('accessories')
...
# DELETE FROM product WHERE cateory_id = 'accessories';
